...
public class KoopaBehaviour : MonoBehaviour, IReset
{
    ...
    {
        if (other.CompareTag("Punch") && GameManager.GetGameManager().GetPlayer().GetComponent<PlayerFSM>().attacking && canBeDamaged)
        {
            fsm.Trigger("Die");
            return;
        }
        if(!other.CompareTag("Player") || !canBeDamaged || other.GetComponent<PlayerFSM>().attacking || fsm.ActiveStateName != "Chase") return;
        fsm.Trigger("Hit");
